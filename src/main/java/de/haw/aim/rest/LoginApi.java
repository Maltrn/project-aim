...
public interface LoginApi {
    ...
    @ApiOperation(value = "", notes = "Meldet den Benutzer (spezifiziert durch Username und Passwort) am System an und liefert einen LoginResponse zurück, der für weitere API Funktionen benötigt wird. Außerdem enthält die Response die ID des Anbieter der dem Benutzer zugeordnet ist und alle dessen Produkt IDs.", response = InlineResponse200.class, tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "LoginResponse, Anbieterinfo ID und alle Produktinfo IDs des Anbieters", response = InlineResponse200.class),
        @ApiResponse(code = 401, message = "Benutzername existiert nicht oder angegebenen Passwort ist falsch", response = InlineResponse200.class),
        @ApiResponse(code = 503, message = "Login nicht erfolgreich, da keine Anbieter und/oder Produktinformationen abgerufen werden können.", response = InlineResponse200.class) })
